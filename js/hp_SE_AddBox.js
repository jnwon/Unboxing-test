nhn.husky.SE_Box = jindo.$Class({
    ...
    $ON_PASTE_BOX : function(){
        this._wrapBlock("BLOCKQUOTE", "se2_quote9");
    },
...
//		var sBookmarkID = oSelection.placeStringBookmark();
		...
		if(nhn.husky.SE2M_Utils.isBlankNode(oFormattingNode)){
			...
			// [SMARTEDITORSUS-1229] 인용구 여러 개 중첩하면 에디터 본문 영역을 벗어나는 현상
			var header = '<select name="category"><option>일기</option><option>영어단어</option><option>그림</option><option>한자</option></select>&nbsp;';
			header += '<input name="title" placeholder="포스트 제목을 입력하세요"><br><br>'
			oFormattingNode.innerHTML = header + '<p style="color:magenta;">박스 바깥에 커서를 위치시킨 후 또 다른 박스를 생성해보세요!</p>';
